...
export async function generateImageAction(
  ...
  const modelInput = input.model.startsWith("zsthimself/")
    ? {
        model: "dev",
        prompt: input.prompt,
        lora_scale: 1,
...
export async function storeImages(data: storeImageInput[]) {
  ...
  for (const img of data) {
    ...
    const { error: dbError, data: dbDate } = await supabase
      .from("generated_images")
      .insert([
        {
          user_id: user.id,
...
export async function deletImage(id: string, imageName: string) {
  const supabase = await createClient();
